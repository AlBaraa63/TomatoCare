"""TomatoCare — Step 2: Train Tomato Gate
Trains the binary Tomato Gate model (tomato vs. other_leaf) to reject non-tomato leaves.
...
def build_tomato_model(num_classes: int) -> tuple:
...
def main() -> None:
    ...
    print(f"--- TomatoCare — Step 2: Training Tomato Gate ({device}) ---")

    config = load_config()
    root = project_root()
    
    data_dir = Path(config["paths"].get("data_dir", str(Path.home() / "tc_data"))) / "stage2_tomato"
    ckpt_dir = root / config["paths"]["checkpoints_dir"]
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    
    ckpt_path = ckpt_dir / "stage2_tomato.keras"
    results_path = root / config["paths"]["results_dir"] / "results_tomato_gate.json"
    ...
    model, base_backbone = build_tomato_model(len(class_names))
    ...
    # 10x slower learning rate (0.0001) to adapt the backbone feature extraction layers to tomato leaf shapes.
    ...
    print(f"Tomato Gate report saved to: {results_path} | Best Val Acc: {best_val_acc*100:.2f}%")
